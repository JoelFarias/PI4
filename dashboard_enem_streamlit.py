...
# -------------------- Carregamento de dados auxiliares (IDH) --------------------
@st.cache_data(show_spinner="Carregando dados de IDH dos municípios...")
def carregar_idh_csv(path='idh_municipios.csv'):
    try:
        hdi_df = pd.read_csv(path, dtype={'codigo_municipio_dv': str})
        hdi_df.columns = [c.lower() for c in hdi_df.columns]
        return hdi_df[['codigo_municipio_dv', 'idh']]
    except Exception:
        return pd.DataFrame(columns=['codigo_municipio_dv', 'idh'])


# -------------------- Decodificadores e pós-processamento --------------------
def decodificar_categorias_enem(df: pd.DataFrame, hdi_df: pd.DataFrame = None) -> pd.DataFrame:
    df = df.copy()
    col_map = {}
    ...
    if hdi_df is not None and 'co_municipio_prova' in df.columns:
        df['co_municipio_prova'] = df['co_municipio_prova'].astype(str).str.strip()
        hdi_df['codigo_municipio_dv'] = hdi_df['codigo_municipio_dv'].astype(str).str.strip()
        df = df.merge(hdi_df, left_on='co_municipio_prova', right_on='codigo_municipio_dv', how='left')
...
def criar_grafico_declaracao_vs_nota(df):
    fig = criar_grafico_barra_renda(df)
    if fig is None or (hasattr(fig, "data") and len(fig.data) == 0):
        ...
        fig.update_traces(marker_line_width=1.5, marker_line_color="white", opacity=0.85)
        fig.update_layout(
...
def criar_grafico_top_municipios_declarados(df, top_n=10):
...
def criar_grafico_pizza(df, column, title, textfont_size=10):
...
def criar_boxplot_nota_media(df):
...
def criar_grafico_barra_renda(df, only_declared=False):
...
def rotulo_curto(s: str, maxlen=18) -> str:
    if pd.isna(s):
        return ''
    if ' - ' in s:
        s2 = s.split(' - ', 1)[1]
...
def criar_matriz_escolaridade_pais_vs_nota(df):
    ...
    x_labels = [rotulo_curto(x) for x in x_labels_full]
    y_labels = [rotulo_curto(y) for y in y_labels_full]
...
def analise_preditiva(df: pd.DataFrame):
    target = 'nota_media_5_notas'
    features = ['faixa_renda', 'escolaridade_pai', 'escolaridade_mae', 'cor_raca', 'sexo', 'idh']
    df_ml = df.dropna(subset=features + [target]).copy()
    df_ml = df_ml[df_ml[target] > 0]
    if len(df_ml) < 100:
        return 0, pd.DataFrame(columns=['Variável', 'Importância', 'Tipo']), 0, 0
    X = df_ml[features]
    y = df_ml[target]
    cat_features = ['faixa_renda', 'escolaridade_pai', 'escolaridade_mae', 'cor_raca', 'sexo']
    num_features = ['idh']
    ...
    feature_names_raw = preprocessor.named_transformers_['onehot'].get_feature_names_out(cat_features)
    feature_names = list(feature_names_raw) + num_features
    importance_rf = list(model_rf.feature_importances_)
    importance_df = pd.DataFrame({
        'Variável_raw': feature_names,
        'Importância': importance_rf
    })
    def map_type(var_code):
    ...
    def get_legible_name(var_code):
        ...
        if var_code.startswith('sexo_'):
            ...
            return f'Sexo: {key}'
        if var_code == 'idh':
            return 'IDH do Município'
        return var_code
    importance_df['Tipo'] = importance_df['Variável_raw'].apply(map_type)
    ...
    return r2_rf, importance_df, r2_lr, len(df_ml)

def criar_mapa_municipios(df):
    if 'latitude' not in df.columns or 'longitude' not in df.columns:
        return None
    df_map = df.groupby(['co_municipio_prova', 'nome_municipio', 'latitude', 'longitude']).agg(
        nota_media=('nota_media_5_notas', 'mean'),
...
def main():
    ...
    df = decodificar_categorias_enem(df_raw, hdi_df=hdi_df)
    ...
    with tab1:
        ...
        with col1:
            fig1 = criar_grafico_pizza(df_filtrado, "escolaridade_pai", "Escolaridade do Pai", textfont_size=10)
            st.plotly_chart(fig1, use_container_width=True)
        with col2:
            fig2 = criar_grafico_pizza(df_filtrado, "escolaridade_mae", "Escolaridade da Mãe", textfont_size=10)
        ...
        with col3:
            fig3 = criar_grafico_barra_renda(df_filtrado)
            pct_unknown = (df_filtrado['faixa_renda_legivel'].fillna('Desconhecido') == 'Desconhecido').mean()
            if fig3 is None or pct_unknown > 0.7:
                fig_alt = criar_grafico_declaracao_vs_nota(df_filtrado)
                st.plotly_chart(fig_alt, use_container_width=True)
            else:
                st.plotly_chart(fig3, use_container_width=True)
        with col4:
            fig4 = criar_boxplot_notas(df_filtrado)
            st.plotly_chart(fig4, use_container_width=True)

        st.markdown("---")

        fig_parent_notes = criar_matriz_escolaridade_pais_vs_nota(df_filtrado)
        ...
        with col_sex:
            fig6 = criar_grafico_pizza(df_filtrado, "sexo", "Distribuição por Sexo", textfont_size=12)
            st.plotly_chart(fig6, use_container_width=True, key="sexo_pie_chart")
        with col_mean:
            fig_mean_box = criar_boxplot_nota_media(df_filtrado)
        ...
        with st.expander("🔍 Detalhamento das Estatísticas"):
            ...
            fig_cor = criar_grafico_pizza(df_filtrado, "cor_raca", "Distribuição por Cor/Raça", textfont_size=11)
        ...
        if 'latitude' in df.columns and 'longitude' in df.columns:
            ...
            if fig_map:
                ...
                st.info("Dados de latitude/longitude não disponíveis para o mapa.")
        else:
            st.info("Dados de latitude/longitude não disponíveis para o mapa.")
    with tab2:
        st.subheader("🔬 Análise Preditiva: Impacto Socioeconômico na Nota Média")
        st.markdown("""O modelo Random Forest Regressor foi treinado para prever a Nota Média usando Renda, Escolaridade dos Pais, Cor/Raça, Sexo e IDH do município.
...
ATENÇÃO: quando a nota individual do participante não existir, utilizamos a média municipal como proxy/contexto (é uma aproximação).""")
        r2_rf, importance_df, r2_lr, n_amostra = analise_preditiva(df_filtrado)
        if r2_rf == 0:
            st.warning("⚠️ Dados insuficientes (menos de 100 registros) para treinar o modelo preditivo com os filtros atuais.")
        else:
            col_r2, col_lr, col_samples = st.columns(3)
            with col_r2:
                st.metric(label="Coeficiente de Determinação RF ($R^2$)", value=f"{r2_rf:.4f}")
            with col_lr:
                st.metric(label="Coeficiente Linear ($R^2$)", value=f"{r2_lr:.4f}")
            with col_samples:
                st.metric(label="Amostra para Predição", value=f"{n_amostra:,}")
